"""Frozen RL-adapter evaluation on all 800 official GenEval2 prompts."""
import argparse
import json
import os
...
from llava.train.rl.geneval2_eval_io import (
    atomic_json, ensure_manifest, load_benchmark, read_record, sha256_file, summarize)
...
def parse_args():
    ...
    p.add_argument('--reward_python', required=True)
...
def main():
    ...
    manifest = dict(protocol='geneval2_full_final_pixel_v1', checkpoint=str(checkpoint),
                    ...
                    reward_model=str(Path(args.reward_model).resolve()), reward='official_soft_tifa_am_and_gm')
    ...
    if rank == 0 and args.report_to == 'wandb':
        ...
        wandb_run = wandb.init(project=os.environ.get('WANDB_PROJECT', 'tar_reasoning'),
                               entity=os.environ.get('WANDB_ENTITY'), job_type='geneval2_full',
    ...
    reward = QwenPixelReward(args.reward_python, args.reward_model, device, args.reward_timeout)
    try:
        ...
        for batch_number, start in enumerate(range(0, len(rows), args.gen_batch_size)):
            ...
            for index, trajectory in zip(indices, batch.trajectories):
                ...
                am, gm, scores = reward.score_images([image], [rows[index]['vqa_list']])
                filename = f'prompt-{index:04d}.png'
                image_path = directory/filename
                temporary = directory/(filename+'.tmp')
                image.save(temporary, format='PNG')
                os.replace(temporary, image_path)
                record = dict(index=index, protocol_id=identity, **rows[index], am=am[0], gm=gm[0],
                              question_scores=scores[0], reflections=trajectory.reflections,
                ...
                print(json.dumps(dict(rank=rank, prompt_index=index, am=am[0], gm=gm[0],
                                      stop=trajectory.stop_reason)), flush=True)
        if world > 1:
            dist.barrier()
        if rank == 0:
            metrics = summarize(directory, rows, manifest)
            print('GENEVAL2_RESULT ' + json.dumps(metrics), flush=True)
            if wandb_run:
                wandb_run.log({**{'geneval2/'+k: metrics[k] for k in ('am','gm','n_prompts')},
                               'geneval2/per_skill_am': metrics['per_skill_am'],
                               'geneval2/per_atomicity': metrics['per_atomicity']})
                table = wandb.Table(columns=['index','prompt','AM','GM','stop','image','reflections'])
                for index in range(8):
                    record = read_record(directory, index, rows[index], identity)
                    table.add_data(index, record['prompt'], record['am'], record['gm'],
                                   record['stop_reason'], wandb.Image(str(directory/record['image'])),
                                   '\n---\n'.join(record['reflections']))
                wandb_run.log({'geneval2/examples': table})
    finally:
        reward.close()
        if wandb_run:
            wandb_run.finish()
